...
public interface UserShareMapper extends BaseMapper<UserShare> {
    // 未登录公共资源（公共）（下载）
    @Select("select share_id, file_name, file_type, user_name, u.created_time as created_time, expire_time " +
    ...
    @Select("select share_id, file_name, file_type, user_name, u.created_time as created_time, expire_time, max_download_times, max_file_dump_times, download_times, file_dump_time " +
            "from user_share s, user u, user_files f " +
            "where s.user_id=u.user_id and s.user_file_id=f.user_file_id and " +
            "u.deleted=false and s.deleted=false and f.deleted=false and " +
    ...
    public UserShareVO getShareDetail(@Param("shareId")Long shareId);
}
